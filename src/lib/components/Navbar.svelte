                    ...
                    <ul class={`w-full flex justify-around mt-4`}>
                        {#each externalAnchors as anchor (anchor.id)}
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                            >{@render anchor.icon(false)}</a>
                        {/each}

                        <button use:toggleTheme>
                            {#if $ThemeStore === 'light'}
                                {@render moonSvg(false)}
                            {:else}
                                {@render sunSvg(false)}
                            {/if}
                        </button>
...
{#snippet moonSvg(small: boolean = true)}
...
{#snippet sunSvg(small: boolean = true)}
...
{#snippet arrowUpSvg(small: boolean = true)}
...
{#snippet linkedinSvg(small: boolean = true)}
...
{#snippet githubSvg(small: boolean = true)}
